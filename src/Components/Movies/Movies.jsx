...
export default function Movies() {
 ...
 <div className="row position-relative mt-3  ">
           {data?data.map((e)=> <div key={e.id} className="col-lg-3 col-md-4 col-sm-6 my-3 cursor position-relative la">
            ...
            <Link to={"/MovieDetails/"+e.id}><div className=' layer  rounded-3 p-3'>{e.overview.split(" ").slice(0,30).join(" ")}</div></Link> 
             {/* <div onClick={()=>addfavorites(e.title,e.poster_path,e.id)} className=' position-absolute fs-3 top-0 star'> <i className="fa-solid fa-star"></i></div> */}
             </div>
